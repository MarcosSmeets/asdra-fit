...
**Decisão:** o app passa a exibir **um** banner (`react-native-google-mobile-ads`), ancorado entre o conteúdo e a barra de abas, presente apenas nas rotas de `(tabs)`. **Formato fixado em `BANNER` 320×50, num slot de 56 dp** — validado no aparelho em 2026-08-03. O `LARGE_ANCHORED_ADAPTIVE_BANNER` (até 150 dp, ~20% da tela) foi testado e recusado por comer a cena do Adari na home; 50 dp é o menor formato que o AdMob oferece. `ads.test.ts` fixa os dois números, para aumentar o anúncio exigir reabrir esta decisão em vez de passar como ajuste de layout. Proibidos: intersticial, anúncio de abertura e **vídeo recompensado**. Consentimento via UMP do Google + ATT no iOS. Unit IDs reais só no perfil EAS `production`; qualquer outro caminho (`__DEV__`, env ausente, env malformado) cai no ID de teste do Google.
